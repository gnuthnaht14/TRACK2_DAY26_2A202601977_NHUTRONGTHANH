...
# Round scale is x1.0 (r1-3), x1.25 (r4-7), x1.5 (r8-10) — a credit saved for
# round 9 buys 1.5x the damage prevention of one spent in round 2, so a flat
# per-round allowance (or `BudgetPacer.is_affordable`'s flat 50% reserve) is
# too conservative late and too loose early. Spending evenly is a mistake
# that looks like discipline. Sums to 96 across 10 rounds — under the
# 100-credit pool even before any `ResultCache` hit or skipped optional call.
...
def round_allowance(round_no: int) -> int:
    """This round's own credit allowance — `ROUND_ALLOWANCE[round_no]`, or a
    conservative `9` (the disciplined ceiling's per-round average) for a
    `round_no` outside `1..10`, which should never happen in a real duel but
    must never raise from inside a JOB 4 (BUDGET) check either."""
    return ROUND_ALLOWANCE.get(round_no, 9)
